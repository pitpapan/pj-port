...
| `CONFIG_VOIP_SIP_DESTINATON_CALL_USER_TAG` | Remote user tag used by selected direct-call paths when the call target is taken from configuration instead of the API argument. This symbol is referenced by the SIP source but is not declared in this module's Kconfig. | external |
...
## Board And Audio Hardware Interface

The SDK contains several board definitions under `boards/arm`, including `iMXRT_1062_Module`, `iMXRT_1064_Module`, and `mimxrt1060_evk_intercom`. The build script does not select one board automatically from the SDK; it expects `--som_name`, then passes that value to `west build -b` as `VAR_SOM_FULL_NAME`.

From the checked-in DTS files, the board that explicitly defines the microphone and speaker devices used by the VOIP audio path is `mimxrt1060_evk_intercom`.

### `mimxrt1060_evk_intercom` Audio Nodes

The intercom board DTS declares these aliases:

| Alias | Device node | Compatible | Purpose |
| --- | --- | --- | --- |
| `audio-recorder` | `audio_recorder` | `adc-edma-audio-recorder` | Microphone capture path. |
| `audio-player` | `audio_player` | `spi-dac-audio-player` | Speaker playback path. |

The microphone input is implemented as an ADC recorder:

- DTS node: `audio_recorder: adc-edma-audio-recorder`.
- Compatible: `adc-edma-audio-recorder`.
- DMA: `edma0`, request source `kDmaRequestMuxADC1`.
- ADC channel: `mic-adc-channel = <15>`.
- Pinctrl: `pinmux_audio_recorder`.
- Pinmux: `iomuxc_gpio_ad_b1_10_adc1_in15`.
- Driver module: `drivers/adc_edma_audio_recorder/zephyr`.

The speaker output is implemented as an SPI DAC player:

- DTS node: `audio_player: lpspi3@40394000`.
- Compatible: `spi-dac-audio-player`.
- Bus/register block: LPSPI3 at `0x40394000`.
- DMA: `edma0`, TX channel configuration from the DTS `dmas` property.
- Pinctrl: `pinmux_audio_player`.
- SPI pinmux: PCS0, SCK, and SDO pins from the LPSPI pin group.
- GPIO controls: `dac-sync-gpios` and `enable-amplifier-gpios`.
- Driver module: `drivers/spi_dac_audio_player/zephyr`.

The `iMXRT_1064_Module` and `iMXRT_1062_Module` board DTS files contain codec-related pinmux entries and comments about custom speaker DAC handling, but their base DTS files do not define the `audio-recorder` and `audio-player` aliases found on `mimxrt1060_evk_intercom`.

### C++ Microphone And Speaker Abstractions

The RTPAudio layer does not use DTS device nodes directly. It receives already-created C++ wrapper objects:

- `RTPAudioStreamer` receives `Peripherals::Microphone *`.
- `RTPAudioReceiver` receives `Peripherals::Speaker *`.

`Peripherals::Microphone` wraps a Zephyr `struct device` and calls a shared `codec_api` interface for recording operations:
...
`Peripherals::Speaker` also wraps a Zephyr `struct device` and calls the same style of `codec_api` interface for playback operations:

- `play_start()`.
- `play_send()`.
- `play_stop()`.
- `play_flush()`.
- tone generation hooks.
- volume control.
- speaker gain and amplifier activation.

So the VOIP media path sees a generic microphone/speaker API, while the intercom board maps that API onto ADC+eDMA input and SPI-DAC+eDMA output.

### Build Dependency To Verify

The ADC recorder and SPI DAC player driver directories exist in `drivers/`, and each has its own CMake/Kconfig gated by `CONFIG_ADC_EDMA_AUDIO_RECORDER` and `CONFIG_SPI_DAC_AUDIO_PLAYER`. In the inspected top-level `drivers/CMakeLists.txt`, those two driver directories are not currently appended to `IMXRT_SDK_DRIVERS`. If this board is built with those devices enabled, verify that the final application or build system adds those driver modules through another path, otherwise the DTS compatibles will not have driver implementations linked into the image.
...
In the current implementation, SIP over TCP is mandatory for the in-house SIP stack. The SIP module builds `SIPTCPSocket.cpp`, `SIPSession` owns a `SIPTcpSocket`, and the socket wrapper creates `SOCK_STREAM` / `IPPROTO_TCP` sockets for both outgoing connections and incoming direct-call listening. There is no SIP UDP transport class, no UDP SIP socket path, and no Kconfig option that switches SIP signaling between TCP and UDP.

This means UDP is currently used for RTP media, not for SIP signaling. Supporting SIP over UDP would be an additional transport feature rather than a configuration change. It would need a new SIP UDP transport wrapper, message datagram handling, retransmission/transaction timers, different timeout behavior, and a policy for whether TCP and UDP can be enabled side by side or whether one transport is selected per session.
...
`SIPIncommingDirectCallSession` acts as a simple TCP server, but its callback sequence is not identical to the registrable incoming-call path:
...
4. It calls `SIPSessionObserver::PendingIncomingCall()` before parsing the `INVITE`, using `CONFIG_VOIP_SIP_DESTINATON_CALL_USER_TAG` as the user tag.
5. If the observer accepts, it resets the SIP receive buffer and waits for an `INVITE`.
6. When the `INVITE` is parsed, the shared `HandleIncommingInvite()` path extracts remote identity and SDP, sends `180 Ringing`, and calls `PendingIncomingCall()` again with the parsed remote user tag.
7. While connected and in call, it runs the in-call state handler.
8. When the call ends or the socket closes, it disconnects and returns to `connecting`.
...
- SIP request parsing is limited to `INVITE`, `ACK`, and `BYE`; other request methods are treated as unsupported or unknown.
- The SIP parser stores headers by exact header name and assumes fields such as `CSeq` are present when parsing a classified message.
...
- SDP parsing keeps media/session parsing state in a static local variable inside `SDP::ParsePayload()`, so repeated SDP parses may depend on previous parser state unless this is corrected.
...
- `CONFIG_ACTIA_CODEC_DEBUG_TXIN` should be treated as unverified in the RTP streamer path: the implementation references a raw debug codec member name that does not match the header declaration.
- The direct incoming-call path can call `PendingIncomingCall()` twice: once before parsing the `INVITE` with the configured destination tag, and once after parsing with the remote tag extracted from SIP.
