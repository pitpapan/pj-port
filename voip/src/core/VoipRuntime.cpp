...
void VoipRuntime::ProcessNotification(const RuntimeNotification &notification) noexcept {
    ...
    case RuntimeNotification::Type::call_rejected:
        ...
        if (error == Error::ok) {
            PublishTransition(transition);
            CompleteCallOperations(*context, notification.error == Error::ok
                                             ? Error::remote_rejected
                                             : notification.error);
            if (scheduler_.IsLive(context->handle) &&
                context->runtime_token != 0)
                (void)adapter_.BeginCallTeardown(context->runtime_token);
            ApplyEffects(effects);
        }
        break;
...
Error VoipRuntime::Dial(AgentHandle agent, const DialRequest &request,
    ...
    *operation = 0;
    ...
    CallContext *context = scheduler_.Resolve(*call);
    if (context != nullptr) {
        context->admitted_at_ms = now_ms_;
        BindCallOperation(*context, id);
        context->signaling_operation = id;
    }
    *operation = id;
    if (call_count_ < calls_.size()) calls_[call_count_++] = *call;
    PublishAdmission(*call, !promoted);
    ApplyEffects(effects);
    RefreshResources();
    return Error::ok;
}
...
Error VoipRuntime::Answer(CallHandle call, OperationId *operation) noexcept {
    ...
    if (!initialized_ || shutting_down_) return Error::shutting_down;
    if (operation == nullptr) return Error::invalid_argument;
    Error error = ReserveOperation(operation);
    if (error != Error::ok) return error;
    CallContext *context = scheduler_.Resolve(call);
    if (context == nullptr) { (void)operations_.RollbackAdmission(*operation); *operation = 0; return Error::invalid_handle; }
    BindCallOperation(*context, *operation);
    context->pending_operation = CallContext::PendingOperation::answer;
    ScheduledTransition transition{};
    error = scheduler_.Answer(call, &transition);
    if (error != Error::ok) { (void)operations_.RollbackAdmission(*operation); *operation = 0; return error; }
    if (!operations_.AcceptAdmission(*operation)) { (void)operations_.RollbackAdmission(*operation); *operation = 0; return Error::internal_failure; }
    if (transition.transition.cause == CallTransition::acceptance) {
        PublishTransition(transition);
        (void)operations_.Complete(*operation, Error::ok);
    }
    RefreshResources();
    return Error::ok;
}
...
Error VoipRuntime::Reject(CallHandle call, std::uint16_t status,
    ...
    CallContext *context = scheduler_.Resolve(call);
    if (context == nullptr) { (void)operations_.RollbackAdmission(*operation); *operation = 0; return Error::invalid_handle; }
    const bool promoted = scheduler_.IsPromoted(call);
    BindCallOperation(*context, *operation);
    const OperationId signaling_operation = context->signaling_operation;
    const OperationId control_operation = *operation;
    context->pending_operation = CallContext::PendingOperation::reject;
    if (promoted) {
        if (!operations_.AcceptAdmission(*operation)) {
            (void)operations_.RollbackAdmission(*operation); *operation = 0;
            return Error::internal_failure;
        }
        if (context->runtime_token != 0)
            (void)adapter_.Reject(context->runtime_token, status);
        RefreshResources();
        return Error::ok;
    }
    ScheduledTransition transition{}; SchedulerEffects effects{};
    error = scheduler_.Reject(call, &transition, effects);
    if (error != Error::ok) { (void)operations_.RollbackAdmission(*operation); *operation = 0; return error; }
    if (!operations_.AcceptAdmission(*operation)) { (void)operations_.RollbackAdmission(*operation); *operation = 0; return Error::internal_failure; }
    PublishTransition(transition);
    if (transition.handle_invalidated) { CompleteOperationIds(signaling_operation, control_operation, Error::remote_rejected); ForgetCall(call); }
    else if (context->runtime_token != 0) (void)adapter_.Reject(context->runtime_token, status);
    ApplyEffects(effects); RefreshResources(); return Error::ok;
}
...
Error VoipRuntime::Cancel(CallHandle call, OperationId *operation) noexcept {
    ...
    if (!initialized_ || shutting_down_) return Error::shutting_down;
    if (operation == nullptr) return Error::invalid_argument;
    Error error = ReserveOperation(operation); if (error != Error::ok) return error;
    CallContext *context = scheduler_.Resolve(call);
    if (context == nullptr) { (void)operations_.RollbackAdmission(*operation); *operation = 0; return Error::invalid_handle; }
    const bool promoted = scheduler_.IsPromoted(call);
    BindCallOperation(*context, *operation);
    ...
    if (promoted) {
        ...
        if (context->runtime_token != 0)
            (void)adapter_.Cancel(context->runtime_token);
        RefreshResources();
        return Error::ok;
    }
    ScheduledTransition transition{}; SchedulerEffects effects{};
    error = scheduler_.Cancel(call, &transition, effects);
    if (error != Error::ok) { (void)operations_.RollbackAdmission(*operation); *operation = 0; return error; }
    if (!operations_.AcceptAdmission(*operation)) { (void)operations_.RollbackAdmission(*operation); *operation = 0; return Error::internal_failure; }
    PublishTransition(transition);
    if (transition.handle_invalidated) { CompleteOperationIds(signaling_operation, control_operation, Error::cancelled); ForgetCall(call); }
    else if (context->runtime_token != 0) (void)adapter_.Cancel(context->runtime_token);
    ApplyEffects(effects); RefreshResources(); return Error::ok;
}
...
Error VoipRuntime::Hangup(CallHandle call, OperationId *operation) noexcept {
    ...
    ScheduledTransition transition{}; SchedulerEffects effects{};
    error = scheduler_.Hangup(call, &transition, effects);
    if (error != Error::ok) { (void)operations_.RollbackAdmission(*operation); *operation = 0; return error; }
    if (!operations_.AcceptAdmission(*operation)) { (void)operations_.RollbackAdmission(*operation); *operation = 0; return Error::internal_failure; }
    PublishTransition(transition);
    if (transition.handle_invalidated) { CompleteOperationIds(signaling_operation, control_operation, Error::ok); ForgetCall(call); }
    else if (context->runtime_token != 0) (void)adapter_.Hangup(context->runtime_token);
    ApplyEffects(effects); RefreshResources(); return Error::ok;
}
...
Error VoipRuntime::SetHeld(CallHandle call, bool held, OperationId *operation) noexcept {
    ...
    ScheduledTransition transition{};
    error = scheduler_.SetHeld(call, held, &transition);
    if (error != Error::ok) { (void)operations_.RollbackAdmission(*operation); *operation = 0; return error; }
    if (!operations_.AcceptAdmission(*operation)) { (void)operations_.RollbackAdmission(*operation); *operation = 0; return Error::internal_failure; }
    PublishTransition(transition);
    if (context->runtime_token != 0) (void)adapter_.SetHeld(context->runtime_token, held);
    (void)operations_.Complete(*operation, Error::ok);
    return Error::ok;
}
...
void VoipRuntime::CancelAllCalls() noexcept {
    for (std::size_t index = call_count_; index != 0; --index) {
        ...
        if (scheduler_.Cancel(handle, &transition, effects) == Error::ok) {
            PublishTransition(transition);
            if (transition.handle_invalidated) {
                CompleteCallOperations(*context, Error::cancelled);
                ForgetCall(handle);
...
Error VoipRuntime::Shutdown() noexcept {
    {
        ...
        if (events_.ReserveServiceStopped(&stopped)) (void)events_.CommitServiceStopped(&stopped);
        stopped_ = true;
        initialized_ = false;
    }
    actor_.Stop();
...
void VoipRuntime::Step(std::uint64_t now_ms) noexcept {
    ...
    while (mailbox_.TryPop(&command)) {
        // The command has already been validated and admitted by the public
        // facade. This drain is deliberately callback-free.
    }
    ApplyTimers(now_ms);
    RuntimeNotification notification{};
    for (std::size_t i = 0; i < FakeRuntimeAdapter::capacity; ++i) {
...
void VoipRuntime::RefreshResources() noexcept {
    ...
    snapshot.available_events = static_cast<std::uint16_t>(VoipEventQueue::ordinary_capacity - events_.Size());
